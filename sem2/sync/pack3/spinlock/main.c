...
void *ascending_routine(void *args) {
    ...
    while (true) {
        my_spinlock_lock(&list->first->sync);
        node_t *prev = list->first, *cur;

        while (prev->next != NULL) {
            cur = prev->next;
            int prev_size = strlen(prev->val);
            my_spinlock_lock(&cur->sync);
            my_spinlock_unlock(&prev->sync);
...
void *swap_routine(void *args) {
    ...
    while (true) {
        ...
        while (prev->next != NULL) {
            ...
            next = cur->next;
            if (next == NULL) {
                my_spinlock_unlock(&cur->sync);
                break;
            }

            my_spinlock_lock(&next->sync);
            prev->next = next;
            my_spinlock_unlock(&prev->sync);
            cur->next = next->next;
            my_spinlock_unlock(&cur->sync);
